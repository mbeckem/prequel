#ifndef EXTPP_BTREE_CURSOR_IPP
#define EXTPP_BTREE_CURSOR_IPP
...
inline void cursor::copy(const cursor& other) {
    ...
    EXTPP_ASSERT(m_tree == other.m_tree, "Cursors must belong to the same tree.");
...
inline void cursor::check_element_valid() const {
...
#ifdef EXTPP_DEBUG
    {
        ...
        EXTPP_ASSERT(m_parents.size() + 1 == height,
        ...
        if (height > 1) {
            ...
            EXTPP_ASSERT(parent_entry.node.get_child(parent_entry.index) == m_leaf.index(),
                         "Must be at that position in the parent.");
        }

        if (height == 1) {
            EXTPP_ASSERT(m_leaf.index() == m_tree->root(), "Leaf must be the root");
        } else if (height > 1) {
            EXTPP_ASSERT(m_parents.front().node.index() == m_tree->root(), "First parent must be the root.");
        } else {
            EXTPP_ASSERT(false, "The tree is empty...");
        }

        EXTPP_ASSERT(m_leaf.valid(), "Leaf must be valid.");
        EXTPP_ASSERT(m_index < m_leaf.get_size(), "Invalid index in leaf.");
    }
#endif
}
...
inline void cursor::init_position() {
    ...
    EXTPP_ASSERT(m_leaf.get_size() > 0, "Leaf cannot be empty.");
