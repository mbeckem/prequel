#ifndef EXTPP_BTREE_TREE_IPP
#define EXTPP_BTREE_TREE_IPP
...
inline void tree::seek_bound(const byte* key, cursor& cursor) const {
    EXTPP_ASSERT(cursor.m_tree == this, "Cursor does not belong to this tree.");
...
inline bool tree::next_leaf(cursor& cursor) const {
    ...
    auto rpos = std::find_if(cursor.m_parents.rbegin(), cursor.m_parents.rend(), [&](const auto& entry) {
...
inline void tree::find(const byte* key, cursor& cursor) const {
    ...
    EXTPP_ASSERT(cursor.m_index < cursor.m_leaf.get_size(), "Invalid index.");
    ...
    if (leaf_size < leaf.max_size()) {
        ...
        if (height() == 1) {
            ...
            for (auto& c : m_cursors) {
                ...
                EXTPP_ASSERT(c.m_leaf.index() == leaf.index(), "Must point to the existing root.");
                EXTPP_ASSERT(c.m_parents.empty(), "There cannot be any internal nodes.");
            ...
            for (auto& c : m_cursors) {
                ...
                if (parent_entry.index == index_in_parent) {
                    EXTPP_ASSERT(c.m_leaf.index() == leaf.index(), "Inconsistent leaf node for parent index.");
...
inline internal_node tree::create_root(block_index left_child,
                                                 block_index right_child,
                                                 const byte* split_key)
{
    // Grow by one level.
...
inline void tree::apply_root_split(const internal_node& new_root,
                                   ...
                                   const internal_node& right_internal)
{
    ...
    for (auto& cursor : m_cursors) {
        ...
        EXTPP_ASSERT(!cursor.m_parents.empty(),
                     "Must have internal nodes on the stack because the root was internal.");
        EXTPP_ASSERT(cursor.m_parents.front().node.index() == left_internal.index(),
...
inline void tree::apply_child_split(const internal_node& parent,
                                    ...
                                    const internal_node& right_internal)
{
    ...
    for (auto& cursor : m_cursors) {
        ...
        EXTPP_ASSERT(parent_stack_index < cursor.m_parents.size(),
                     "Parent stack index out of bounds.");
        EXTPP_ASSERT(children_stack_index >= 1 && children_stack_index < cursor.m_parents.size(),
...
inline void tree::erase(cursor& cursor) {
    EXTPP_ASSERT(!(cursor.m_flags & cursor.INVALID), "Cursor must not be invalid.");
    EXTPP_ASSERT(!(cursor.m_flags & cursor.DELETED), "Cursor must not point to a deleted element.");
    EXTPP_ASSERT(cursor.m_parents.size() == this->height() - 1, "Not enough nodes on the parent stack.");
    ...
    if (leaf.get_size() >= leaf.min_size())
        return;
    EXTPP_ASSERT(cursor.m_parents.size() > 0, "Must have parents.");
    const internal_node& parent = cursor.m_parents.back().node;
    const u32 index_in_parent = cursor.m_parents.back().index;
...
inline void tree::propagate_leaf_deletion(cursor& cursor,
                                          block_index child_node,
                                          u32 child_node_index)
{
    EXTPP_ASSERT(!cursor.m_parents.empty(), "There must be internal node parents.");
    ...
    while (1) {
        EXTPP_ASSERT(cursor.m_parents.at(stack_index).node.index() == node.index(),
        ...
        if (stack_index == 0) {
            ...
            if (node.get_child_count() == 1) {
                ...
                for (auto& c : m_cursors) {
                    if (c.invalid())
                        continue;
                    EXTPP_ASSERT(!c.m_parents.empty() && c.m_parents[0].node.index() == node.index(),
...
inline void tree::steal_leaf_entry(const internal_node& parent,
...
inline void tree::steal_internal_entry(const internal_node& parent, u32 stack_index,
...
inline void tree::merge_leaf(const internal_node& parent,
                             const leaf_node& leaf, u32 leaf_index,
                             const leaf_node& neighbor, u32 neighbor_index) {
    const u32 parent_children = parent.get_child_count();
    const u32 leaf_size = leaf.get_size();
...
// Note: stack_index is the index of the children (node and neighbor).
inline void tree::merge_internal(const internal_node& parent, u32 stack_index,
                                 const internal_node& node, u32 node_index,
                                 const internal_node& neighbor, u32 neighbor_index)
{
    const u32 parent_children = parent.get_child_count();
...
#define ERROR(...) EXTPP_THROW( \
    ...
    struct checker {
        ...
        checker(const class tree* tree): tree(tree) {}
