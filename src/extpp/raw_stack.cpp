...
public:
    ...
    raw_stack_node(block_handle block, u32 value_size, u32 capacity)
        ...
        , m_capacity(capacity)
    {
        EXTPP_ASSERT(m_capacity == compute_capacity(this->block().block_size(), m_value_size),
    ...
    const byte* get(u32 index) const {
        EXTPP_ASSERT(index < get_size(), "Index out of bounds.");
    ...
    , m_node_capacity(raw_stack_node::compute_capacity(get_engine().block_size(), m_value_size))
{
    if (m_node_capacity == 0)
        EXTPP_THROW(bad_argument("Block size is too small to fit a single value."));
...
void raw_stack_impl::top(byte* value) const {
    if (!value)
        EXTPP_THROW(bad_argument("Invalid value buffer."));
    if (empty())
        EXTPP_THROW(bad_operation("The stack is empty."));
...
void raw_stack_impl::pop() {
    ...
    EXTPP_ASSERT(!m_buf.front().empty(), "The first buffered block cannot be empty.");
...
void raw_stack_impl::free_node(block_index index) {
    EXTPP_ASSERT(nodes() > 0, "Inconsistent node counter.");
...
#ifdef EXTPP_DEBUG
    if (empty()) {
        EXTPP_ASSERT(m_buf.empty(), "No blocks can be buffered.");
        return;
    }

    if (nodes() == 1) {
        EXTPP_ASSERT(m_buf[0] && m_buf[0].index() == top_node(),
                "Must buffer the last node.");
        EXTPP_ASSERT(m_buf.size() == 1,
                     "Must not buffer any other node.");
        return;
    }

    EXTPP_ASSERT(m_buf.size() == 2, "Must buffer two nodes.");
    EXTPP_ASSERT(m_buf[1] && m_buf[1].index() == top_node(),
            "Must buffer the last node.");
    EXTPP_ASSERT(m_buf[0] && m_buf[0].index() == m_buf[1].get_prev(),
            "Must buffer the second to last node.");
    EXTPP_ASSERT(m_buf[0].index() == m_buf[1].get_prev(),
...
detail::raw_stack_impl& raw_stack::impl() const {
    if (!m_impl)
        EXTPP_THROW(bad_operation("Invalid stack instance."));
    return *m_impl;
}
