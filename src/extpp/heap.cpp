...
public:
    ...
    slot(u32 offset, u32 size)
        ...
        , m_size(size)
    {
        EXTPP_ASSERT(offset > 0, "Invalid offset.");
    }

    bool valid() const { return m_offset != 0; }

    u32 offset() const {
        EXTPP_ASSERT(valid(), "slot must be valid.");
        return m_offset;
    }
...
public:
    ...
    {
        EXTPP_ASSERT(m_block.valid(), "Invalid block.");

        if (m_block.block_size() <= 128)
            EXTPP_THROW(bad_argument("Block sizes this small are not supported."));
    }
    ...
    u32 unfragmented_free_bytes() const {
        ...
        EXTPP_ASSERT(header.free_ptr >= front_used, "Corrupted free pointer.");
    ...
    u32 free_bytes() const {
        ...
        EXTPP_ASSERT(free <= m_block.block_size()
...
public:
    ...
    static constexpr u32 offset_to_slot(u32 offset) {
        EXTPP_ASSERT(offset >= serialized_size<page_header>(),
                     "Offset lies in page header.");
        offset -= serialized_size<page_header>();
        EXTPP_ASSERT(offset % serialized_size<slot>() == 0,
...
heap_reference heap::allocate(const byte* object, u32 object_size) {
    if (object == nullptr) {
        EXTPP_THROW(bad_argument("Invalid data pointer."));
    }
...
heap_reference heap::allocate_impl(const byte* object, u32 object_size) {
    if (object_size == 0) {
        EXTPP_THROW(bad_argument("Zero sized allocation."));
    }
...
void heap::free(heap_reference ref) {
    ...
    EXTPP_ASSERT(objects_count() > 0, "Corrupted objects count.");
    EXTPP_ASSERT(objects_size() >= objects_size(), "Corrupted objects size.");
    set_objects_count(objects_count() - 1);
    set_objects_size(objects_size() - object_size);
}
...
u32 heap::size(heap_reference ref) const {
    ...
    visitor v;
    visit_object(ref, v);
    EXTPP_ASSERT(v.result != 0, "Zero result cannot happen.");
    return v.result;
}
...
void heap::load(heap_reference ref, byte* buffer, u32 buffer_size) const {
    struct visitor {
        ...
        void small_object(const page_handle& page, u32 offset, u32 size) {
            if (size != buffer_size) {
                EXTPP_THROW(bad_argument("Size of the buffer does not match the object's size."));
            }
...
void heap::validate() const {
    ...
    for (pc.move_min(); pc; pc.move_next()) {
        ...
        if (!entry.large_object()) {
            ...
            if (!fc.find(free_map_entry(entry.block(), available))) {
                EXTPP_THROW(corruption_error("Unable to find free map entry for a heap page."));
            }
    ...
    for (fc.move_min(); fc; fc.move_next()) {
        ...
        if (!pc.find(free_entry.block)) {
            EXTPP_THROW(corruption_error("Unable to find page entry for heap page in free map."));
        }

        page_entry entry = pc.get();
        if (entry.large_object()) {
            EXTPP_THROW(corruption_error("Large object allocations cannot have free space."));
        }

        page_handle page = get_engine().read(entry.block());
        if (page.effective_free_bytes() != free_entry.available) {
            EXTPP_THROW(corruption_error("Wrong number of available bytes in heap page."));
        }
    }

    if (computed_byte_size != byte_size()) {
        EXTPP_THROW(corruption_error("Wrong byte size."));
    }
    if (computed_object_count != objects_count()) {
        EXTPP_THROW(corruption_error("Wrong number of objects."));
    }
    if (computed_object_size != objects_size()) {
        EXTPP_THROW(corruption_error("Wrong size of objects."));
    }
}
...
void heap::visit_object(heap_reference ref, ObjectVisitor&& visitor) const {
    ...
    // TODO: Introduce a runtime debugging flag for checked derefencing (this is slow)
    {
        ...
        if (!cursor) {
            EXTPP_THROW(bad_argument("Corrupted reference, the storage is not in use by this heap."));
        }
        if (cursor.get().large_object() != ref.is_large_object()) {
            EXTPP_THROW(bad_argument("Corrupted reference, the storage type is unexpected."));
        }
    }
    ...
    if (ref.is_small_object()) {
        ...
        if (slot_index >= header.slot_count) {
            EXTPP_THROW(bad_argument("Corrupted reference, invalid slot index."));
        }

        const slot slt = page.get_slot(slot_index);
        visitor.small_object(page, slt.offset(), slt.size());
    } else {
        EXTPP_ASSERT(get_engine().to_offset(address) == 0, "Must be at the start of the block.");
...
raw_address heap::allocate_large_object(const byte* object, u32 object_size) {
    ...
    EXTPP_ASSERT(page_insert_result.inserted, "The block index was not unique.");
...
void heap::free_large_object(raw_address addr) {
    EXTPP_ASSERT(addr.valid(), "Address must be valid.");
    ...
    if (!entry.large_object()) {
        EXTPP_THROW(bad_argument("Corrupted reference does not match the storage's index entry."));
    }
...
raw_address heap::allocate_small_object(const byte* object, u32 object_size) {
    ...
    auto init_at_slot = [&](page_handle& page, u32 slot_index) {
        const slot slt = page.get_slot(slot_index);
        EXTPP_ASSERT(slt.valid(), "Slot must be valid.");
        EXTPP_ASSERT(slt.size() == object_size, "Invalid size for new allocation.");
        if (object) {
    ...
    // have to be revisited.
    {
        ...
        if (auto free_entry_pos = m_free_map.lower_bound(search_key); free_entry_pos) {
            EXTPP_ASSERT(free_entry_pos.get().available >= object_size,
            ...
            EXTPP_ASSERT(page.effective_free_bytes() == free_entry_pos.get().available,
...
void heap::free_small_object(raw_address slot_address) {
    ...
    EXTPP_ASSERT(size_cursor, "Matching free map entry must exist.");
    ...
    if (page.get_header().slot_count == 0) {
        ...
        EXTPP_ASSERT(inserted, "Must have been inserted successfully.");
        unused(inserted);
    }
}
...
u32 heap::page_allocate(page_handle& page, u32 object_size) {
    EXTPP_ASSERT(page.effective_free_bytes() >= object_size,
    ...
    if (required_size > page.unfragmented_free_bytes()) {
        page_compact(page);
        EXTPP_ASSERT(required_size <= page.unfragmented_free_bytes(),
    ...
    const u32 slot_index = [&]() {
        ...
        EXTPP_ASSERT(page.unfragmented_free_bytes() >= serialized_size<slot>(),
...
void heap::page_free(page_handle& page, u32 slot_index) {
    page_header header = page.get_header();
    if (slot_index >= header.slot_count) {
        EXTPP_THROW(bad_argument("Invalid object address (possible double free or data corruption)."));
    }
...
void heap::free_blocks(block_index index, u64 n) {
    ...
    EXTPP_ASSERT(blocks_count() >= n, "Corrupted blocks count.");
    set_blocks_count(blocks_count() - n);
}
