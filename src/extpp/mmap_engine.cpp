...
public:
    ...
    void clear() {
        EXTPP_ASSERT(m_refcount == 0, "Instance must not be in use.");
    ...
    void set(u64 index, byte* data) {
        EXTPP_ASSERT(m_refcount == 0, "Instance must not be in use.");
        EXTPP_ASSERT(index != block_index::invalid_value, "Invalid index.");
        EXTPP_ASSERT(data, "Invalid address.");
...
void mmap_handle::handle_unref() {
    EXTPP_ASSERT(m_refcount > 0, "Invalid refcount.");
    if (--m_refcount == 0) {
    ...
    , m_pool(1024 /* max size */)
{
    if (mmap_chunk_size % m_block_size != 0) {
        EXTPP_THROW(bad_argument("mmap chunk size must be a multiple of the block size."));
    }
...
byte* mmap_engine_impl::access_block(u64 block_index) const {
    ...
    EXTPP_ASSERT(index_of_chunk < m_mappings.size(), "Chunk index out of bounds.");
