...
id_generator::value_type id_generator::allocate() {
    if (m_tree.empty()) {
        ...
        if (max() == std::numeric_limits<value_type>::max()) {
            PREQUEL_THROW(exception("ID space exhausted."));
        }
...
void id_generator::free(value_type id) {
    PREQUEL_CHECK(id > 0 && id <= max(), "Invalid id.");
    ...
    if (left_interval && left_interval->end >= id) {
        PREQUEL_THROW(bad_argument("ID has already been freed."));
    }
    ...
    if (range.end == max()) {
        ...
        unused(inserted);
        PREQUEL_ASSERT(inserted, "Interval must have been inserted.");
    }
}
...
id_generator::value_type id_generator::pop_one() {
    PREQUEL_ASSERT(!m_tree.empty(), "Tree must not be empty.");
