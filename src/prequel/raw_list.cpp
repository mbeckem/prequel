...
void raw_list_impl::pop_back() {
    if (empty())
        PREQUEL_THROW(bad_operation("The list is empty."));
...
void raw_list_impl::insert_first(const byte* value) {
    PREQUEL_ASSERT(empty(), "list must be empty");
...
void raw_list_impl::erase_at(raw_list_node node, u32 index) {
    PREQUEL_ASSERT(node.valid(), "Invalid node.");
    PREQUEL_ASSERT(index < node.get_size(), "Index out of bounds.");
...
static void check_cursor_valid(const raw_list_cursor_impl& c) {
    if (!c.list)
        PREQUEL_THROW(bad_cursor("The cursor's list instance has been destroyed."));
}
...
static void check_cursor_valid_element(const raw_list_cursor_impl& c) {
    ...
    if (c.flags & raw_list_cursor_impl::INVALID)
        PREQUEL_THROW(bad_cursor("Invalid cursor."));

    PREQUEL_ASSERT(c.node.valid(), "Invalid node.");
...
detail::raw_list_impl& raw_list::impl() const {
    if (!m_impl)
        PREQUEL_THROW(bad_operation("Invalid list instance."));
    return *m_impl;
}
