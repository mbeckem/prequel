#ifndef PREQUEL_DETAIL_INLINED_ANY_HPP
#define PREQUEL_DETAIL_INLINED_ANY_HPP
...
private:
    ...
    auto* construct(Args&&... args) {
        ...
        PREQUEL_ASSERT(!m_vtable, "instance must be empty.");
    ...
    void copy_construct(detail::inlined_any_vtable* vtable, const void* from) {
        PREQUEL_ASSERT(!m_vtable, "instance must be empty.");
        PREQUEL_ASSERT(vtable, "new vtable is null");
    ...
    static auto& inner_ref(Any& self) {
        ...
        if (!ptr) {
            PREQUEL_THROW(bad_operation("The any object does not store an object of this type."));
        }
        return *ptr;
    }
