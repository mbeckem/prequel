#ifndef PREQUEL_EXCEPTION_HPP
#define PREQUEL_EXCEPTION_HPP
 ...
 */
#define PREQUEL_AUGMENT_EXCEPTION(e) \
    (::prequel::detail::with_location((e), PREQUEL_SOURCE_LOCATION))
 ...
 */
#define PREQUEL_THROW_NESTED(e) (::std::throw_with_nested(PREQUEL_AUGMENT_EXCEPTION(e)))
...
Exception with_location(Exception&& e, const source_location& where) {
    static_assert(std::is_base_of<exception, std::decay_t<Exception>>::value,
                  "Exception must be derived from prequel::exception.");
    e.set_where(where);
...
public:
     ...
     * @ref PREQUEL_THROW or @ref PREQUEL_THROW_NESTED, otherwise `where()`
...
/**
 * Thrown when an allocation using prequel::allocator failed.
 */
