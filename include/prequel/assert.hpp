...
#define PREQUEL_CONSTEXPR_ASSERT(cond, message)                           \
    do {                                                                \
        if (!(cond)) {                                                  \
            throw ::prequel::detail::assertion_failure_impl_(             \
                __FILE__, __LINE__, #cond, (message));                  \
        }                                                               \
    } while (0)
...
#define PREQUEL_UNREACHABLE(message) (::prequel::detail::unreachable_impl_(__FILE__, __LINE__, (message)))
